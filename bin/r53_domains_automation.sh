...
files_to_clean=("akamai-domains.csv" "no-akamai-domains.csv" "zoneIDs" "output.csv" "domains.csv")
...
while IFS= read -r line; do
        ...
        if [[ "$list_dns" == *"acm-validation"* ]]; then
                echo "$domain,$list_dns,ACM Validation" >> output.csv
        elif [[ "$list_dns" =~ sendgrid ]]; then
                echo "$domain,$list_dns,Sendgrid" >> output.csv
        elif [[ "$list_dns" =~ cloudfront ]]; then
                echo "$domain,$list_dns,CloudFront" >> output.csv
        elif [[ "$list_dns" =~ ^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
                echo "$domain,$list_dns,IP Address sendgrid?" >> output.csv
        elif [[ "$list_dns" == *"domainconnect"* || "$list_dns" == *"domainkey"* ]]; then
                echo "$domain,$list_dns,Domain Control" >> output.csv
		elif [[ "$list_dns" == *"edgekey.net"* ]]; then
                echo "$domain,$list_dns,Behind WAF" >> output.csv
        else
                behind_vpn=false
                for ip in "${ips[@]}"; do
                        if grep -q "^${ip}$" vpn_ips.txt; then
                                echo "$domain,$list_dns,Behind VPN" >> output.csv
                                behind_vpn=true
                                break
                        fi
                done
                if [ "$behind_vpn" = false ]; then
                        echo "$domain,$list_dns,Exposed" >> output.csv
                fi
        fi
...
while IFS= read -r line; do
		...
		matching_line=$(grep "$base_domain" output.csv)
        if [[ "$matching_line" == *"edgekey.net"* ]]; then
                echo "$line,WAF duplicate" >> output.csv
        else
                echo "$line,Exposed" >> output.csv
        fi
